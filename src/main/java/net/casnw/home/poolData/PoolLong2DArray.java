 ...
 *
 */
public class PoolLong2DArray implements Attributeable.Long2DArrayable {

    long[][] value = null;
    public int colsNum = 0;
    public int rowsNum = 0;

    public PoolLong2DArray() {
    }

    public PoolLong2DArray(String value) {
        if (value != null && !"".equalsIgnoreCase(value)) {
            setValue(value);
        }
    }

    public PoolLong2DArray(long[][] value) {
        setValue(value);
    }
    ...
    public void setValue(long[][] value) {
        if (value != null) {
            ...
            this.value = new long[this.rowsNum][];
            for (int i = 0; i < this.rowsNum; i++) {
                int col = value[i].length;
                this.value[i] = new long[col];
                for (int j = 0; j < col; j++) {
    ...
    @Override
    public long[] getRowValue(int index) {
        if (value != null) {
    ...
    @Override
    public long[] getColValue(int index) {
        long[] colValue;
        if (value != null) {
            if (index >= 0 && index < this.colsNum) {
                colValue = new long[this.rowsNum];
                for (int i = 0; i < this.rowsNum; i++) {
    ...
    @Override
    public long getCellValue(int row, int col) {
        long cellValue = 0;
        if (value != null) {
            ...
            throw new NullPointerException("PoolLong2DArray is null!");
        }
        return cellValue;
    }

    @Override
    public void setCellValue(int row, int col, long cellValue) {
        if (value != null) {
    ...
    public void setValue(String value) {
        if (value != null && !value.equalsIgnoreCase("")) {
            ...
            for (int i = 0; i < this.rowsNum; i++) {
                ...
                long[] rowValue = new long[currentColsNum];
                for (int j = 0; j < currentColsNum; j++) {
                    rowValue[j] = Long.parseLong(cellString[j].trim());
                }
                valueList.add(rowValue);
            }
            //将List<long[]>组装成二维数组
            long[][] value2D = new long[this.rowsNum][this.colsNum];
            for (int i = 0; i < valueList.size(); i++) {
                long[] temp = valueList.get(i);
                if (temp.length == this.colsNum) {
                    value2D[i] = temp;
                } else if (temp.length < this.colsNum) {
                    long[] temp2 = new long[this.colsNum];
    ...
    public long[] convert2DTo1DCol() {
        long[] a = new long[rowsNum * colsNum];
        if (value != null) {
    ...
    public void set1DTo2DArr(long[] value) {
        if (this.value == null) {
            this.value = new long[rowsNum][colsNum];
        }
        for (int i = 0; i < colsNum; i++) {
            for (int j = 0; j < rowsNum; j++) {
